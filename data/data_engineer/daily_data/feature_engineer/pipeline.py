...
class FeatureEngineeringPipeline:
    ...
    def __init__(
        ...
        Args:
            time_column: 时间列名
            categorical_columns: 类别特征列名列表
            numerical_columns: 数值特征列名列表
            use_cyclical_encoding: 是否使用周期性编码
            create_derived_features: 是否创建派生特征
    ...
    def split_train_test(
        ...
        print(f"训练集: {len(train_df):,} 条 ({train_df[self.time_column].min()} 至 {train_df[self.time_column].max()})")
        print(f"测试集: {len(test_df):,} 条 ({test_df[self.time_column].min()} 至 {test_df[self.time_column].max()})")
    ...
    def fit_transform_train(self, train_df: pd.DataFrame) -> pd.DataFrame:
        ...
        derived_numerical = [
            ...
            if feat in df.columns and not feat.startswith('is_') and '_level' not in feat
        ]
        all_numerical_cols.extend(derived_numerical)
        
        # 添加时间周期编码特征
        if self.use_cyclical_encoding:
            cyclical_features = [col for col in df.columns if col.endswith('_sin') or col.endswith('_cos')]
    ...
    def transform_test(self, test_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.is_fitted:
            raise ValueError("流水线未拟合，请先对训练集调用 fit_transform_train()")
    ...
    def save_processed_data(
        ...
        print(f"训练集已保存: {train_path} ({len(train_df):,} 行 × {len(train_df.columns)} 列)")
        print(f"测试集已保存: {test_path} ({len(test_df):,} 行 × {len(test_df.columns)} 列)")
        ...
        if self.numerical_scaler:
            ...
            self.numerical_scaler.save(scaler_path)
    ...
    def _quality_check(self, df: pd.DataFrame, dataset_name: str) -> None:
        ...
        if constant_cols:
            print(f"\n  警告: {dataset_name}存在常量列: {constant_cols}")
        else:
            print(f"  ✓ 无常量列")
    ...
    def get_summary(self) -> dict:
        ...
        summary = {
            "时间特征": self.time_extractor.get_feature_names() if self.time_extractor else [],
            "派生特征": self.feature_creator.get_feature_names() if self.feature_creator else [],
            "类别特征": {},
            "标准化方法": self.scaling_method
        }
        
        if self.categorical_encoder:
            summary["类别特征"] = self.categorical_encoder.get_info()
    ...
    def print_summary(self) -> None:
        ...
        print(f"\n时间特征 ({len(summary['时间特征'])} 个):")
        for feat in summary['时间特征']:
            print(f"  - {feat}")
        
        print(f"\n派生特征 ({len(summary['派生特征'])} 个):")
        for feat in summary['派生特征']:
            print(f"  - {feat}")
